...
async function processarVendaAutomatica(whatsapp_id, descricao, dados) {
  ...
  try {
    ...
    if (prodRes.rows.length === 0)
      throw new Error(`Produto "${dados.item}" não cadastrado`);

    const produto = prodRes.rows[0];
    const total = dados.qtd * (dados.valor || produto.preco);

    const novoEstoque = produto.estoque - dados.qtd;
    await client.query("UPDATE produtos SET estoque = $1 WHERE id = $2", [
      novoEstoque,
      produto.id,
    ]);

    const catRes = await client.query(
      "SELECT id FROM categorias WHERE nome = 'Venda direta' LIMIT 1",
    );
    ...
    return {
      total,
      novoEstoque,
      alerta: novoEstoque <= 5 ? "⚠️ Estoque baixo!" : null,
    };
  } catch (e) {
    await client.query("ROLLBACK");
    throw e;
